...
pub trait LinkMapBacked<'a>: Sized {
    ...
    fn rel_dyn(&'a self) -> Option<&'a [unix::ElfRela]> {
        ...
        let entries = (rela.d_val + self.dynamic_load_address()) as *const unix::ElfRela;
        let entries = unsafe { std::slice::from_raw_parts(entries, entries_count) };
        Some(entries)
    }

    #[cfg(target_pointer_width = "64")]
    fn plt_dyn(&'a self) -> Option<&'a [unix::ElfRela]> {
        ...
        let relaent = std::mem::size_of::<unix::ElfRela>();
        let entries_count = relasz / relaent;
        let entries = (rela.d_val + self.dynamic_load_address()) as *const unix::ElfRela;
        let entries = unsafe { std::slice::from_raw_parts(entries, entries_count) };
        Some(entries)
    ...
    fn rel_dyn(&'a self) -> Option<&'a [unix::ElfRel]> {
        ...
        let entries_count = relasz / relaent;
        let entries = (rela.d_val + self.dynamic_load_address()) as *const unix::ElfRel;
        let entries = unsafe { std::slice::from_raw_parts(entries, entries_count) };
        Some(entries)
    }

    #[cfg(target_pointer_width = "32")]
    fn plt_dyn(&'a self) -> Option<&'a [unix::ElfRel]> {
        let rela = self.try_find_section(SectionType::DT_JMPREL)?;
        let relasz = self.try_find_section(SectionType::DT_PLTRELSZ)?.d_val as usize;
        let relaent = std::mem::size_of::<unix::ElfRel>();
        let entries_count = relasz / relaent;
        let entries = (rela.d_val + self.dynamic_load_address()) as *const unix::ElfRel;
        let entries = unsafe { std::slice::from_raw_parts(entries, entries_count) };
        Some(entries)
